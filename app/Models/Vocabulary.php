...
class Vocabulary extends CommonModel
{
    ...
    public function search($search_params, $search_word, $search_date_since, $search_date_until, $offset = null, $limit = null, $orderParam = null, $orderAsc = 'desc')
    {
        ...
        //各検索項目による検索
        if(isset($search_params['vocabulary_code'])) {
            $result = $result->where('vocabulary_code', 'like', '%'.$search_params['vocabulary_code'].'%');
        }
...
//        }
        ...
        if($search_word != '' && $search_word != null) {
            ...
            $result = $result->where(function ($query) use($search_word) {
                $query
                    ->orWhere('vocabulary_code', 'like', '%'.$search_word.'%')
    ...
    public function setData($data)
    {
        ...
        foreach($data as $key => $param) {
            $insert_data[] = array(
                'vocabulary_code' => $param['vocabulary_code'],
                'module' => $param['module'],
                'grade' => $param['grade'],
                'stage' => $param['stage'],
