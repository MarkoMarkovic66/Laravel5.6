...
class GslWordFreq extends CommonModel
{
    public $table = 'gsl_word_freqs';
    public $name = 'GSL単語頻度';
    public $primary = 'id';
    ...
    public function search($search_params, $search_word, $search_date_since, $search_date_until, $offset = null, $limit = null, $orderParam = null, $orderAsc = 'desc')
    {
        ...
        //各検索項目による検索
        if(isset($search_params['freq'])) {
            $result = $result->where('freq', '=', $search_params['freq']);
        }
        if(isset($search_params['word'])) {
            $result = $result->where('word', 'like', '%'.$search_params['word'].'%');
        }
        ...
        if($search_word != '' && $search_word != null) {
            ...
            $result = $result->where(function ($query) use($search_word) {
                $query
                    ->orWhere('freq', 'like', '%'.$search_word.'%')
                    ->orWhere('word', 'like', '%'.$search_word.'%');
            });
        }
