...
AlueSubsys.TaskCalendarUtils = {
    ...
    onChangeTaskToReserveTask : function(e) {
        ...
        }).done(function (response)
        {
            ...
            var url = AlueSubsys.TaskCalendarUtils.URL_REDIRECT_TASK_CALENDAR_TOP + userId;
       ...
       // var search_tips       = $(searchPanel).find('input[name="tips"]').val();
        ...
        }).done(function (response)
        {
            ...
            $.each(taskList, function(idx, row){
                taskListHtml += AlueSubsys.TaskCalendarUtils.createSearchResultRowHtml(idx, row);
            });
            ...
            //生成したDOMにイベントを付与する
            AlueSubsys.TaskCalendarUtils.setEventHandlerAfterMasterSearch();
    ...
    setEventHandlerAfterMasterSearch : function() {
        ...
        .on('click', function(e){ //問題切り替えボタンクリック
            ...
            //問題をマスタから切り替える処理を実行
            AlueSubsys.TaskCalendarUtils.changeTaskToNewMasterData(qtype, userId, userTaskPeriodId, orgTaskId, taskMasterId);
        });
    },
    //問題をマスタから切り替える処理
    ...
    setDataToTaskAssignModal : function(qType, userId, selectedTaskList, reservedTaskList) {
                ...
                $.each(taskListPerCell.taskList, function(idxQuestion, taskListPerQuestion) {
                    ...
                    if(dayNumberBreak != dayNumber){
                        ...
                        htmlText += '<td rowspan="' + rowCountPerDay + '"  class="td-center">'
                                 + AlueSubsys.Utils.getDayDisplay(dayNumber)
                                 + '</td>';
                    }
        ...
        $('table.task-assign-table select.select-reserve-task').on('change', function(e){
            AlueSubsys.TaskCalendarUtils.onChangeTaskToReserveTask(e);
        });
        $('table.task-assign-table button.btn-mst-change-task').on('click', function(e){
            AlueSubsys.TaskCalendarUtils.openModalToChangeMasterTask(e);
        });
    },
    ...
    openDetailModal : function(dayNumber, cellNumber, taskTypeName, jsonQuestion){
        ...
        var title = AlueSubsys.Utils.getDayDisplay(dayNumber) + '曜日の '
                    + taskTypeName + ' 問題詳細';
    ...
    openTaskAssignModal : function(userId, qType){
        ...
        }).done(function (response)
        {
            ...
            //モーダルにデータをセット
            AlueSubsys.TaskCalendarUtils.setDataToTaskAssignModal(qType, userId, selectedTaskList, reservedTaskList);
    ...
    registEventHandler : function(){
        ...
        $('div[name^="div-btn-cal-detail"]').on('click', function(e){
            ...
            AlueSubsys.TaskCalendarUtils.openDetailModal(dayNumber, cellNumber, taskTypeName, jsonQuestion);
            return false;
        });
        ...
        $('div[name="div-btn-modal-grammar"]').on('click', function(e){
            var userId = $(document).find('input[name="userId"]').val();
            var qType = 'Grammar';
            AlueSubsys.TaskCalendarUtils.openTaskAssignModal(userId, qType);
            return false;
        });
        //Vocabulary問題一覧表示ボタン
        ...
        $('select[name^="select-task-type"]').on('change', function(e){
            ...
            AlueSubsys.TaskCalendarUtils.changeTaskType(userId, dayNumber, cellNumber, newTaskType);
            return false;
        });

        //G/V出題予定モーダル：マスタから切り替えモーダル：検索
        $('#cMasterModal div.modal-body div.c-master-modal-search .btn-search').on('click', function(e){
            AlueSubsys.TaskCalendarUtils.searchGVmaster();
            return false;
        });
...
$(document).ready( function(){
    //初期化
    AlueSubsys.TaskCalendarUtils.init();
});
