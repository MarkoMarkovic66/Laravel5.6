 ...
 * 会員詳細 javascript
 */
AlueIntegOffice.MemberDetailUtils = {
    ...
    registUserLearningPolicy : function(){
        ...
        $.ajax({
            type : 'POST',
            url  : AlueIntegOffice.MemberDetailUtils.URL_LEARNING_POLICY_REGIST,
            dataType : 'JSON',
            data : postData
        }).done(function (response)
        {
    ...
    pageReload : function() {
        ...
        AlueIntegOffice.AjaxUtils.postForm(AlueIntegOffice.MemberDetailUtils.URL_MEMBER_DETAIL, postData);
        return false;
    },
    ...
    openTaskDetailModal : function(userTaskId) {
        ...
        var userTaskLogsJsonObj = AlueIntegOffice.MemberDetailUtils.userTaskLogsJson;
    ...
    lessonReport : function(userId){
        ...
        AlueIntegOffice.AjaxUtils.submitPostForm(this.URL_MEMBER_REPORT, postData, 'memberReport');
        return false;
    },
    ...
    registEventHandler : function(){
        ...
        //    AlueIntegOffice.MemberDetailUtils.updateUserBasicInfo();
        //});

        $('button[name="create-lp-btn"]').on('click', function(){
            AlueIntegOffice.MemberDetailUtils.registUserLearningPolicy();
        });

        $('button[name="closeTaskDetailModal"]').on('click', function(e){
            AlueIntegOffice.MemberDetailUtils.closeTaskDetailModal();
        });
        $('button[name="modal-btn-answer-download"]').on('click', function(e){
            var answerUrl = $(e.target).data('answer-url');
            AlueIntegOffice.MemberDetailUtils.voiceDataDownload(answerUrl);
        });

        //学習方針登録モーダルの閉時処理（データをリセットする）
        $(document).on('shown.bs.modal', '#newPolicyModal', function(){
            $(this).find('input[name="lp-created_at"]').val(AlueIntegOffice.Utils.getNow());
        ...
        $('div#userOutputLogsDiv').on('scroll', function(){
            ...
            if(AlueIntegOffice.MemberDetailUtils.onScrollingUserOutputLogs){
            ...
            if(outerBottom <= outerScrollTop) {
                ...
                //スクロール処理の処理中をセット
                AlueIntegOffice.MemberDetailUtils.onScrollingUserOutputLogs = true;
                //データの動的取得：OUTPUT履歴
                AlueIntegOffice.MemberDetailUtils.getUserOutputLogs();
            }
            return false;
        });
        ...
        $('div#userLessonLogsDiv').on('scroll', function(){
            ...
            if(AlueIntegOffice.MemberDetailUtils.onScrollingUserLessonLogs){
                return false;//スクロール処理中(データ取得処理中)であれば何もしない
            }
            ...
            if(outerBottom <= outerScrollTop) {
                ...
                //スクロール処理の処理中をセット
                AlueIntegOffice.MemberDetailUtils.onScrollingUserLessonLogs = true;
                //データの動的取得：レッスン受講状況
                AlueIntegOffice.MemberDetailUtils.getUserLessonLogs();
            }
            return false;
        });
        ...
        $('div#userTaskLogsDiv').on('scroll', function(){
            ...
            if(AlueIntegOffice.MemberDetailUtils.onScrollingUserTaskLogs){
                return false;//スクロール処理中(データ取得処理中)であれば何もしない
            }
            ...
            if(outerBottom <= outerScrollTop) {
                ...
                //スクロール処理の処理中をセット
                AlueIntegOffice.MemberDetailUtils.onScrollingUserTaskLogs = true;
                //データの動的取得：レッスン受講状況
                AlueIntegOffice.MemberDetailUtils.getUserTaskLogs();
            }
            return false;
        });
        ...
        $('div#userMessageLogsDiv').on('scroll', function(){
            ...
            if(AlueIntegOffice.MemberDetailUtils.onScrollingUserMessageLogs){
                return false;//スクロール処理中(データ取得処理中)であれば何もしない
            }
            ...
            if(outerBottom <= outerScrollTop) {
                ...
                //スクロール処理の処理中をセット
                AlueIntegOffice.MemberDetailUtils.onScrollingUserMessageLogs = true;
                //データの動的取得：メッセージ送受信履歴
                AlueIntegOffice.MemberDetailUtils.getUserMessageLogs();
            }
            return false;
        });
        ***/
        ...
        $('select[name="lp-category1"]').on('change', function(){
            ...
            $.ajax({
                type : 'GET',
                url  : AlueIntegOffice.MemberDetailUtils.URL_GET_LEARNING_POLICY_CATEGORY,
                dataType : 'JSON',
                data : postData
            }).done(function (response)
            {
    ...
    getUserOutputLogs : function(){
        var skip = AlueIntegOffice.MemberDetailUtils.skipRowCountUserOutputLogs;
        AlueIntegOffice.MemberDetailUtils.ajaxGetDataAndDraw('UserOutputLogs', skip);
    },
    //データの動的取得：レッスン受講状況
    getUserLessonLogs : function(){
        var skip = AlueIntegOffice.MemberDetailUtils.skipRowCountUserLessonLogs;
        AlueIntegOffice.MemberDetailUtils.ajaxGetDataAndDraw('UserLessonLogs', skip);
    },
    //データの動的取得：宿題実施状況
    getUserTaskLogs : function(){
        var skip = AlueIntegOffice.MemberDetailUtils.skipRowCountUserTaskLogs;
        AlueIntegOffice.MemberDetailUtils.ajaxGetDataAndDraw('UserTaskLogs', skip);
    },
    //データの動的取得：メッセージ送受信履歴
    getUserMessageLogs : function(){
        var skip = AlueIntegOffice.MemberDetailUtils.skipRowCountUserMessageLogs;
        AlueIntegOffice.MemberDetailUtils.ajaxGetDataAndDraw('UserMessageLogs', skip);
    },
    ...
    ajaxGetDataAndDraw : function(dataType, skip){
        ...
        //データLoading行の表示
        AlueIntegOffice.MemberDetailUtils.dispDataLoading(dataType, true);
        ...
        {
            //データLoading行の表示解除
            AlueIntegOffice.MemberDetailUtils.dispDataLoading(dataType, false);

            if(!response){
                return false;
            }
            //取得したデータの描画
            AlueIntegOffice.MemberDetailUtils.ajaxDrawData(dataType, response.data);
    ...
    ajaxDrawData : function(dataType, responseData){
        ...
        if(dataType === 'UserOutputLogs'){
            ...
            //データskip数を保持しておく
            AlueIntegOffice.MemberDetailUtils.skipRowCountUserOutputLogs = currentCount;
            //スクロール処理の完了をセット
            AlueIntegOffice.MemberDetailUtils.onScrollingUserOutputLogs = false;
            ...
            //データskip数を保持しておく
            AlueIntegOffice.MemberDetailUtils.skipRowCountUserLessonLogs = currentCount;
            //スクロール処理の完了をセット
            AlueIntegOffice.MemberDetailUtils.onScrollingUserLessonLogs = false;
            ...
            $.each(dataRows, function(idx, row){
                ...
                rowHtml += '<tr>'
                        ...
                        + '<td class="basic-table-btn">'
                            ...
                            + 'onclick="AlueIntegOffice.MemberDetailUtils.openTaskDetailModal('+ row.userTaskId + ');" '
                            + '>詳細</button>'
                        + '</td>'
                        + '</tr>';
            });
            ...
            AlueIntegOffice.MemberDetailUtils.userTaskLogsJson =
                        AlueIntegOffice.MemberDetailUtils.userTaskLogsJson.concat(dataRows);

            //データ全件数を表示
            $('span#userTaskLogsTotalCount').html('全件数: '+ totalCount + '件');
            //データskip数を保持しておく
            AlueIntegOffice.MemberDetailUtils.skipRowCountUserTaskLogs = currentCount;
            //スクロール処理の完了をセット
            AlueIntegOffice.MemberDetailUtils.onScrollingUserTaskLogs = false;
            ...
            //データskip数を保持しておく
            AlueIntegOffice.MemberDetailUtils.skipRowCountUserMessageLogs = currentCount;
            //スクロール処理の完了をセット
            AlueIntegOffice.MemberDetailUtils.onScrollingUserMessageLogs = false;
        }
    },
...
$(document).ready( function(){
    //初期化
    AlueIntegOffice.MemberDetailUtils.init();
